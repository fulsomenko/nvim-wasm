...
async function startNvim({ inputBuffer, cols, rows }) {
  ...
  // TUI mode: forward raw ANSI output to main thread
  const stdoutFd = new SinkFd((chunk) => {
    postMessage({ type: "tui-output", data: chunk });
  });

  const stderrFd = new SinkFd((chunk) => {
    // Forward stderr too (useful for debugging)
    postMessage({ type: "tui-output", data: chunk });
  });
  ...
  // TUI mode: no --embed flag, with TERM and size env vars
  const args = ["nvim", "-u", "NORC", "--noplugin", "-i", "NONE", "-n"];
